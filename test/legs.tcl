# Test delle tratte SENZA episodio: si forza la tratta 3 (verso le
# Sirene, niente sbarco); l'arrivo deve far ripartire il viaggio
# verso Scilla (leg 4, phase 0). Gli arrivi CON episodio sono
# coperti da episode/circe/eolo.tcl (leg 0/1/2).
...
after time 3 { debug write memory 0xC040 3 }
...
after time 8 {
    if {[catch {
        ...
        if {$l0 == 3} {
            report "tratta forzata: OK (leg=3, verso le Sirene)"
        } else {
            report [format "tratta forzata: leg=%d (atteso 3)" $l0]
        }
    ...
    after time 12 {
        if {[catch {
            ...
            if {$l1 == 4 && $md == 0 && $ph == 0} {
                report "tratta successiva: OK (leg=4, verso Scilla)"
            } else {
                report [format "tratta successiva: leg=%d mode=%d phase=%d (attesi 4,0,0)" \
                    $l1 $md $ph]
            }
